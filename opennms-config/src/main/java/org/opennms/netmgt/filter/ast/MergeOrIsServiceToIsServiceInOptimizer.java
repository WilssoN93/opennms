...
/** OR of IsServiceExpr only → IsServiceInExpr. */
final class MergeOrIsServiceToIsServiceInOptimizer extends BaseOptimizerStep {
    ...
    protected Expr rewrite(Expr e) {
        if (e instanceof OrExpr) {
            ...
            if (serviceNames.size() == rewritten.size() && serviceNames.size() >= 2) {
