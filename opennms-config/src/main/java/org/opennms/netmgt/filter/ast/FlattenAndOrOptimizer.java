...
/** Flattens nested AND/OR: (a AND b) AND c → a AND b AND c; (a OR b) OR c → a OR b OR c. */
final class FlattenAndOrOptimizer extends BaseOptimizerStep {
    ...
    protected Expr rewrite(Expr e) {
        if (e instanceof AndExpr) {
            ...
            return flattened.size() == 1 ? flattened.get(0) : new AndExpr(flattened);
        }
