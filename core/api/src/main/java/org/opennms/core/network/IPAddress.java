...
public class IPAddress implements Comparable<IPAddress> {
    ...
    public IPAddress(final String dottedNotation) {
        if (dottedNotation != null) {
            ...
            if (ipv4Bytes != null) {
                m_addressBytes = ipv4Bytes;
                m_inetAddress = getInetAddress(ipv4Bytes);
                return;
            }
        }
    ...
    /**
     * Parse a strict dotted-decimal IPv4 literal without invoking the JDK name service.
     *
     * @return four address octets, or {@code null} if the input requires
     *         {@link InetAddress#getByName(String)}
     */
    public static byte[] parseIpv4LiteralBytes(final String dottedNotation) {
        return tryParseIpv4Bytes(dottedNotation);
    }
